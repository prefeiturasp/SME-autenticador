...
public partial class WebControls_PessoaDocumento_UCGridPessoaDocumento : MotherUserControl
{
    ...
    public DataTable _VS_tipos_documentos
    {
        get
        {
            ...
            {
                ViewState["_VS_tipos_documentos"] = SYS_TipoDocumentacaoBO.GetSelect(Guid.Empty, "", "", 0, false, 1, 1);
            }
    ...
    protected void rptDocumento_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
                ...
                (e.Item.ItemType == ListItemType.Item))
        {
            ...
            if (lblUnf_idEmissao != null && !string.IsNullOrEmpty(lblUnf_idEmissao.Text))
                ((DropDownList)e.Item.FindControl("ddlUF")).SelectedValue = lblUnf_idEmissao.Text;
            ...
            {
                //CASO UMA NOVA LINHA SEJA ADICIONADA, SOMENTE MOSTRA OS TIPOS DE DOCUMENTOS COM SITUAÇÃO 1 ("Não")
                var bloqueiaTiposDocumentos = _VS_tipos_documentos.AsEnumerable().Where(x => x.Field<string>("tdo_situacao") == "Sim");

                bloqueiaTiposDocumentos.ToList().ForEach(x =>
                                     ((DropDownList)e.
                                     Item.FindControl("ddlTipoDoc"))
                                     .Items.FindByValue(Convert.ToString(x.Field<Guid>("tdo_id"))).Enabled = false
                                    );
               
            }

            if (lblPai_idOrigem != null && !string.IsNullOrEmpty(lblPai_idOrigem.Text))
                ((DropDownList)e.Item.FindControl("ddlPais")).SelectedValue = lblPai_idOrigem.Text;
